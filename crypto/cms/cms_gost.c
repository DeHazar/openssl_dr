...
static int gost_cms_encrypt(CMS_RecipientInfo *ri)
{
    ...
    /* Create ASN1_OBJECT for the parameter */
    ...
    /*
     * Set keyEncryptionAlgorithm to kexp15 with the appropriate parameter.
     *
     * The X509_ALGOR structure takes ownership of param_obj on success,
     * so we don't need to free it in the success path.
     *
     * Format: SEQUENCE {
     *   algorithm    OBJECT IDENTIFIER, -- kexp15 OID
     *   parameters   OBJECT IDENTIFIER  -- agreement OID
     * }
     */
    if (!X509_ALGOR_set0(alg, OBJ_nid2obj(kexp15_oid),
                         V_ASN1_OBJECT, param_obj)) {
